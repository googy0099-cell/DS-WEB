...
import {
  ...
  getPrintMethod, setPrintMethod, isAndroid, rawbtEnabled,
  htmlToPng, printImageViaRawbt,
} from "@/lib/thermal-print";
import type { PrintMethod } from "@/lib/thermal-print";
...
export default function AdminSettingsPage() {
  ...
  const [printMethod, setPrintMethodState] = useState<PrintMethod>("auto");
  ...
  useEffect(() => {
    ...
    setPrintMethodState(getPrintMethod());
  ...
  function handleMethodChange(m: PrintMethod) {
    setPrintMethodState(m);
    setPrintMethod(m);
  }

  // Sample receipt HTML built from the current (unsaved) form values — the
  // same buildReceiptHtml used for real receipts, so the test matches exactly.
  function sampleReceiptHtml(): string {
    return buildReceiptHtml(
      ...
      {
        shopName: rShopName, shopInfo: rShopInfo, paperWidth: rPaperWidth, footer: rFooter,
        logoUrl: rLogoUrl || undefined, showOrderId: rShowOrderId, showDate: rShowDate,
        showCustomer: rShowCustomer, showNote: rShowNote, showItemPrice: rShowItemPrice,
        showTotal: rShowTotal, titleSize: rTitleSize, feedLines: rFeedLines,
        headerAlign: rHeaderAlign, htmlFontSize: rHtmlFontSize, logoSize: rLogoSize,
      }
    );
  }

  async function handleRawbtTestPrint() {
    printImageViaRawbt(await htmlToPng(sampleReceiptHtml()));
  }
  ...
  async function testReceiptPrint() {
    const html = sampleReceiptHtml();

    // RawBT (Android) — render to image and print, no dialog
    if (rawbtEnabled()) {
      printImageViaRawbt(await htmlToPng(html));
      return;
    }

    // Try serial — silent print if printer connected
    const hasPrinter = await getGrantedPrinter();
    if (hasPrinter) {
      setSerialTesting(true); setSerialStatus("");
      const ok = await printToSerial(buildReceiptEscPos(
        {
    ...
    // Fallback: browser print window — same HTML as the real receipt
  ...
  async function testKitchenPrint() {
    const html = buildKitchenHtml(
      {
        orderId: 999, orderName: "ทดสอบ", note: "ไม่ใส่น้ำตาล", tableId: 3,
        items: [
          { nameTh: "ชาไทย", selectedSize: "XL", selectedAddons: null, selectedOptions: null, quantity: 2 },
          { nameTh: "กาแฟ", selectedSize: null, selectedAddons: null, selectedOptions: null, quantity: 1 },
        ],
      },
      { paperWidth: kPaperWidth, showTable: kShowTable, showNote: kShowNote }
    );
    ...
    const hasPrinter = await getGrantedPrinter();
    if (hasPrinter) {
      const ok = await printToSerial(buildKitchenEscPos(
        {
          id: 999, orderName: "ทดสอบ", totalTHB: 150, note: "ไม่ใส่น้ำตาล",
        ...
        { showTable: kShowTable, showNote: kShowNote }
      ));
      if (ok) return;
    }

    // Fallback: browser print window — same HTML as the real kitchen ticket
    ...
    <div className="max-w-lg space-y-8">
      ...
      <div>
        ...
        <div className="bg-white rounded-2xl shadow-sm p-5 space-y-4 mb-4">
          ...
          {/* Print method */}
          <div>
            <label className="text-sm font-semibold text-navy block mb-2">วิธีพิมพ์</label>
            <div className="grid grid-cols-2 gap-2">
              {([
                { v: "auto", label: "อัตโนมัติ", hint: onAndroid ? "เครื่องนี้ → RawBT" : "เครื่องนี้ → Serial" },
                { v: "rawbt", label: "RawBT (บลูทูธ)", hint: "มือถือ/แท็บเล็ต Android" },
                { v: "serial", label: "Serial (USB)", hint: "Chrome/Edge บนคอม" },
                { v: "browser", label: "หน้าต่าง browser", hint: "เลือกเครื่องพิมพ์เอง" },
              ] as { v: PrintMethod; label: string; hint: string }[]).map((opt) => (
                <button key={opt.v} onClick={() => handleMethodChange(opt.v)}
                  className={`text-left px-3 py-2 rounded-xl border-2 transition-all ${printMethod === opt.v ? "border-orange bg-orange/10" : "border-sand"}`}>
                  <div className={`text-sm font-semibold ${printMethod === opt.v ? "text-orange" : "text-navy"}`}>{opt.label}</div>
                  <div className="text-[11px] text-gray-400">{opt.hint}</div>
                </button>
              ))}
            </div>
            {(printMethod === "rawbt" || (printMethod === "auto" && onAndroid)) && (
              <div className="mt-3 bg-sky-50 border border-sky-200 rounded-xl p-3 space-y-2">
                <p className="text-xs text-sky-800">
                  พิมพ์ผ่านแอป <b>RawBT</b> ตรงเข้าเครื่องพิมพ์บลูทูธ — กดพิมพ์แล้วออกเลย ไม่มีหน้าต่างเลือกเครื่องพิมพ์ (ต้องติดตั้งแอป RawBT และจับคู่เครื่องพิมพ์ไว้ในแอปก่อน)
                </p>
                <button onClick={handleRawbtTestPrint}
                  className="w-full bg-sky-600 text-white font-semibold py-2.5 rounded-xl text-sm hover:bg-sky-700">
                  🖨️ ทดสอบพิมพ์ผ่าน RawBT
                </button>
              </div>
            )}
          </div>
