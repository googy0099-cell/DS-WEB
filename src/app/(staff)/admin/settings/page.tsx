...
import {
  ...
  buildReceiptLines, buildKitchenLines, renderLinesPng, printImageViaRawbt,
...
export default function AdminSettingsPage() {
  ...
  function handleRawbtTestPrint() {
    const sample: Parameters<typeof buildReceiptLines>[0] = {
    ...
    printImageViaRawbt(renderLinesPng(lines, rPaperWidth));
  }
  ...
  async function testReceiptPrint() {
    ...
    if (rawbtEnabled()) {
      printImageViaRawbt(renderLinesPng(buildReceiptLines(sampleReceipt, escSettings), rPaperWidth));
      return;
    }
    ...
    if (hasPrinter) {
      ...
      const ok = await printToSerial(buildReceiptEscPos(sampleReceipt, escSettings));
...
<div class="footer">${rFooter}</div>
  ...
  async function testKitchenPrint() {
    ...
    // RawBT (Android) — render to image and print, no dialog
    if (rawbtEnabled()) {
      printImageViaRawbt(renderLinesPng(buildKitchenLines(sampleKitchen, { showTable: kShowTable, showNote: kShowNote }), kPaperWidth));
      return;
    }
    ...
    const hasPrinter = await getGrantedPrinter();
    if (hasPrinter) {
      const ok = await printToSerial(buildKitchenEscPos(sampleKitchen, { showTable: kShowTable, showNote: kShowNote }));
      if (ok) return;
    }
