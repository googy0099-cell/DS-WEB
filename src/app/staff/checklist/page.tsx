...
export default function ChecklistPage() {
  ...
  // Countdown timer for time-limited checklists
  useEffect(() => {
    if (!checklist?.timeLimitMinutes || !checklist.startedAt) {
      setRemainingSeconds(null);
      return;
    }
    const deadline = new Date(checklist.startedAt).getTime() + checklist.timeLimitMinutes * 60_000;
    const calc = () => Math.max(0, Math.round((deadline - Date.now()) / 1000));
    setRemainingSeconds(calc());
    const id = setInterval(() => setRemainingSeconds(calc()), 1000);
    return () => clearInterval(id);
  }, [checklist?.timeLimitMinutes, checklist?.startedAt]);
      ...
      {type && (
        <>
          ...
          {remainingSeconds !== null && !allDone && (
            <div className={`flex items-center gap-2 mb-5 px-3 py-2 rounded-xl text-sm font-semibold ${
              remainingSeconds === 0
                ? "bg-red-500/15 text-red-400"
                : remainingSeconds < 300
                ? "bg-amber-500/15 text-amber-400"
                : "bg-white/5 text-[#f8f1e5]/60"
            }`}>
              <span>{remainingSeconds === 0 ? "⏰" : "⏱"}</span>
              <span>{remainingSeconds === 0 ? "เกินเวลาแล้ว — อาจถูกหักเงิน" : `เหลือเวลา ${formatRemaining(remainingSeconds)}`}</span>
            </div>
          )}
          {checklist?.deductionApplied && (
            <div className="flex items-center gap-2 mb-5 px-3 py-2 rounded-xl bg-red-500/15 text-red-400 text-sm font-semibold">
              <span>⚠️</span>
              <span>ถูกหักเงินเนื่องจากทำเช็คลิสต์ไม่ทันเวลา</span>
            </div>
          )}
