...
export function buildReceiptHtml(data: ReceiptHtmlData, settings: ReceiptHtmlSettings): string {
  ...
  // Blank feed AFTER the cut line (must be last). Each line is a real white-filled
  // box (~5mm) — real white pixels in the html2canvas PNG, so the thermal printer
  // advances the paper through it. 3 always-on base lines (~15mm) guarantee the
  // content clears the print-head→tear-bar dead zone; the slider adds more.
  const feedLines = Math.max(0, Math.min(10, settings.feedLines ?? 3));
  const feedHtml = Array.from({ length: feedLines + 3 },
