...
export function buildReceiptHtml(data: ReceiptHtmlData, settings: ReceiptHtmlSettings): string {
  ...
  // Blank feed after the cut line, in real text lines so html2canvas (RawBT image
  // print) reliably captures the whitespace — an empty div gets trimmed from the PNG.
  const feedLines = Math.max(0, Math.min(10, settings.feedLines ?? 3));
  const feedLineH = fs + 2;
  const feedHtml = feedLines > 0
    ? `<div class="feed" style="height:${feedLines * feedLineH}px;line-height:${feedLineH}px">${"&nbsp;<br/>".repeat(feedLines)}</div>`
    : "";
