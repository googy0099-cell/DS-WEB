...
<style>@page{margin:0;size:${w} auto}*{margin:0;padding:0;box-sizing:border-box}body{font-family:'Sarabun','Helvetica Neue',Arial,sans-serif;font-size:${fs}px;color:#111;width:${w};margin:0 auto;padding:3mm 4mm}.logo{display:block;max-width:${logoSz}px;max-height:${logoSz}px;margin:0 auto 4px;object-fit:contain}h1{font-size:${Math.round(fs * 1.4)}px;font-weight:900;text-align:${hAlign};margin-bottom:2px}.sub{font-size:${Math.round(fs * 0.85)}px;text-align:${hAlign};color:#555;margin-bottom:4px}.divider{border:none;border-top:1px dashed #aaa;margin:4px 0}.receipt-no{text-align:center;font-size:11px;color:#999;margin-bottom:4px;letter-spacing:0.05em}table{width:100%;border-collapse:collapse}.total-row td{font-weight:bold;font-size:${Math.round(fs * 1.15)}px;padding-top:4px;border-top:1px dashed #aaa}.note{font-size:${Math.round(fs * 0.92)}px;margin-top:4px}.footer{text-align:center;font-size:${Math.round(fs * 0.85)}px;color:#777;margin-top:6px}@media print{button{display:none}}</style>
</head><body>
