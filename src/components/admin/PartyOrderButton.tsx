...
/**
 * Party order button: creates a PENDING TAB order linked to a specific bill.
 * Unlike CashierOrderButton, this does NOT set source:"cashier" so the order
 * appears as a new alert in the dashboard — staff must accept it before it
 * auto-groups into the party bill checkout.
 */
export default function PartyOrderButton({ billId, billName, onCreated, triggerClassName }: {
  billId: number;
  billName: string;
  onCreated?: () => void;
  ...
  useEffect(() => {
    if (!memberInfo) { setSessionLink(null); return; }
    fetch(`/api/pos/sessions/lookup?billId=${billId}&userId=${memberInfo.id}`)
      .then((r) => r.json())
  ...
  useEffect(() => {
    if (!open) return;
    fetch(`/api/pos/bills/${billId}/players`)
      .then((r) => r.json())
  ...
  function closeModal() {
    ...
    setMemberCode(""); setMemberInfo(null); setMemberError(""); setSessionLink(null);
    setBillPlayers([]); setSelectedPlayerId("");
  }
  ...
  async function submit() {
    ...
    setSaving(true);
    // No source:"cashier" → order is PENDING + no handledById → triggers alert for staff to accept
    ...
    if (res.ok) {
      ...
      // Always TAB — grouped with the party bill at checkout
      ...
      {open && (
        <div className="fixed inset-0 bg-black/70 z-50 flex items-end md:items-center justify-center">
          <div className="bg-white rounded-t-2xl md:rounded-2xl w-full md:max-w-lg flex flex-col max-h-[90dvh]">
            <div className="flex items-center justify-between px-5 py-4 border-b border-sand shrink-0">
              <div>
                <h3 className="font-bold text-navy text-lg">🛒 สั่งอาหาร</h3>
                <p className="text-xs text-gray-400 mt-0.5">ตี้ {billName} · รวมบิลตอนเช็กเอาท์</p>
              </div>
            ...
            <div className="flex-1 overflow-y-scroll overscroll-contain p-4 space-y-4">
              ...
              <div>
                ...
                <input value={orderName} onChange={(e) => setOrderName(e.target.value)}
                  placeholder={billPlayers.length > 0 ? "หรือพิมพ์ชื่อใหม่" : "เช่น คุณสมชาย หรือ โต๊ะ 3"}
                  className="w-full border-2 border-sand rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:border-orange" />
              </div>
              ...
              {/* Info banner: always TAB */}
              <div className="bg-amber-50 border border-amber-200 rounded-xl px-3 py-2.5 text-xs text-amber-700">
                🧾 ออเดอร์นี้จะรวมกับตี้ <span className="font-bold">{billName}</span> และชำระรวมตอนเช็กเอาท์
              </div>
